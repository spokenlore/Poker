...
def displayCombinations(outputfile):
	with open(outputfile, 'w') as output:
		output.write("%s: %d : %.5f percent\n" % ("Royal Flushes", RoyalFlush, float(RoyalFlush)/float(TotalHands)*100))
		output.write("%s: %d : %.5f percent\n" % ("Straight Flushes", StraightFlush, float(StraightFlush)/float(TotalHands)*100))
		output.write("%s: %d : %.5f percent\n" % ("Quadruples", Quads, float(StraightFlush)/float(TotalHands)*100))
		output.write("%s: %d : %.5f percent\n" % ("Full Houses", FullHouse, float(FullHouse)/float(TotalHands)*100))
		output.write("%s: %d : %.5f percent\n" % ("Flushes", Flush, float(Flush)/float(TotalHands)*100))
		output.write("%s: %d : %.5f percent\n" % ("Straights", Straight, float(Straight)/float(TotalHands)*100))
		output.write("%s: %d : %.5f percent\n" % ("Triples", Triples, float(Triples)/float(TotalHands)*100))
		output.write("%s: %d : %.5f percent\n" % ("Two Pairs", TwoPairs, float(TwoPairs)/float(TotalHands)*100))
		output.write("%s: %d : %.5f percent\n" % ("Pairs", Pairs, float(Pairs)/float(TotalHands)*100))
		output.write("%s: %d : %.5f percent\n" % ("High Cards", HighCard, float(HighCard)/float(TotalHands)*100))
		output.write("%s: %d" % ("Total Hands", TotalHands))
